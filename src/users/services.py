...
class UserService(BaseService[UserModel]):
    ...
    async def authenticate(self, email: str, password: str) -> Optional[
        UserModel]:
        user = await self.get_by_email(email=email)
        if user is None or not verify_password(plain_password=password,
...
class UserAddressService(BaseService[UserAddress, UserAddress]):
    ...
    async def get_user_address_by_id(self, address_id: int,
                                     user_id: int) -> UserAddress:
...
def get_user_address_service(repo: UserAddressRepository = Depends(
    UserAddressRepository)) -> UserAddressService:
