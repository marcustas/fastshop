...
user_router = APIRouter(prefix=UserManagementRoutesPrefixes.user_address)
...
async def product_list(
        product_service: Annotated[get_product_service, Depends()]) -> list[
    ProductModel]:
    """
...
async def user_address_list(
        ...
        user_address_service=Depends(get_user_address_service)) -> List[
    UserAddress]:
    """
        ...
        """
    return await user_address_service.get_user_addresses(
...
async def user_address_detail(
    ...
    """
    result = await user_address_service.get_user_address_by_id(address_id=pk,
                                                               user_id=current_user.id)
    if result is None:
